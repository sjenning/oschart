...
type Factory interface {
	...
	// KubeClient returns a ConfigV1 client. It uses the following priority to specify the cluster
	// configuration: --kubeconfig flag, KUBECONFIG environment variable, in-cluster configuration.
	Client() (config.Interface, error)
...
func (f *factory) Client() (config.Interface, error) {
	clientConfig, err := Config(f.kubeconfig, f.baseName)
	if err != nil {
		return nil, err
	}

	client, err := config.NewForConfig(clientConfig)
	if err != nil {
		return nil, err
	}
	return client, nil
